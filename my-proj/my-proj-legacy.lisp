...
(defun-g draw-verts-frag-stage ((uv :vec2)  &uniform (sam :sampler-2d))
  ;; (v! 1 1 1 0)
  ;; (texture sam (rtg-math.vector2:* uv (v! 10 10))))
  (texture sam uv))
...
(defun draw ()
  ...
  ;; (print (skitter:mouse-down-p 0))

  ;; (print (skitter:mouse-pos (mouse)))

  (when
      (skitter:mouse-down-p 0)
    (print "hi"))
...
(defun init ()
         ...
         (labels
           ...
           (sort
            (uiop:directory-files
             (make-pathname
              :directory  '(:relative "assets" "FXpack13" "Effect1")))
            (lambda (it other)
  ...
  ;; (setf *fireball-samplers*
  ;;       (map 'list (lambda (it)
  ;;                    (sample it :wrap :clamp-to-border))
  ;;            *fireball-tex*))
